...
UPDATE_FREQ  = 20       # update the actor and critic networks every 'UPDATE_EVERY' time steps
...
class Agent():
    ...
    def __init__(self, state_size, action_size, random_seed):
        ...
        # Replay memory to store  past events
        self.memory = ReplayBuffer(action_size, BUFFER_SIZE, BATCH_SIZE, random_seed)
        
        #nint step number for update frequency 
        self.time_step = 0
    ...
    def step(self, state, action, reward, next_state, done):
        ...
        self.time_step = (self.time_step + 1) % UPDATE_FREQ # Increase step
        if self.time_step == 0: # Do not update every step
    ...
    def act(self, state, add_noise=True):
        ...
        if add_noise:  ## add noise to boost exploration
